...
path = r'./data/Boulder/Exp_1_2_train'
...
path = r'./data/Boulder/Exp_1_2_test'
all_files = glob.glob(path + "/*.dat")
all_files.sort()
...
fig.savefig('./RNN Paper Results/Exp1_2/Boulder/4hour_Figure 2.jpg', bbox_inches = 'tight')
...
plt.savefig('./RNN Paper Results/Exp1_2/Boulder/4hour_Figure 3', bbox_inches='tight')
...
for i in levels_index[0]:
    for j in levels_index[1]:
        df_new_train.loc[i].loc[j]['Kt'] = df_new_train.loc[i].loc[j]['Kt'].shift(-4)
...
for i in levels_index2[0]:
    for j in levels_index2[1]:
        df_new_test.loc[i].loc[j]['Kt'] = df_new_test.loc[i].loc[j]['Kt'].shift(-4)
...
df_trainLoss.to_csv('RNN Paper Results/Exp1_2/Boulder_4hour_1000Epoch_TrainLoss.csv')
df_testLoss = pd.DataFrame(data={'Test Loss':test_loss, 'iteration': test_iter}, columns=['Test Loss','iteration'])
df_testLoss.to_csv('RNN Paper Results/Exp1_2/Boulder_4hour_1000EpochTestLoss.csv')
